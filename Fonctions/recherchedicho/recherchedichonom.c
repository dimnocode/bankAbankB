...
int recherchedichonom(struct clientA trinomA[],int taille,char *nom);
...
int recherchedichonom(struct clientA trinomA[],int taille,char *nom)//fonction qui recherche un nom dans un tableau trié pour les nom 
{
    ...
    {
        i=(bg+bd)/2;
        if(stringcomp(trinomA[i].nom,nom) == -1)
            bd = i-1;
        else
            if(stringcomp(trinomA[i].nom,nom)==1)
                bg=i+1;
...
printf("\n%d",recherchedichonom(trinomA,nbClientA,sourceB[i].nom));
}
